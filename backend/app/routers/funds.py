...
logger = logging.getLogger("fund.funds")

@router.put("/sort")
async def update_sort(payload: FundSortRequest) -> dict:
    """更新基金的自定义排序顺序。"""
    t0 = time.time()
    logger.info("update_sort: start sorting %d codes", len(payload.codes))
    ...
    logger.info("update_sort: sorting completed, elapsed: %.3fs", time.time() - t0)
...
async def get_fund(code: str) -> dict:
    ...
    t0 = time.time()
    logger.info("get_fund: start request code=%s", code)
    try:
    ...
    logger.info("get_fund: completed for %s, elapsed: %.3fs", code, time.time() - t0)
...
async def get_nav(code: str, days: int = 60) -> dict:
    ...
    t0 = time.time()
    logger.info("get_nav: start request code=%s, days=%d", code, days)
    days = max(1, min(days, 1500))
    records = await nav_cache.get_nav_history(code, days=days)
    logger.info("get_nav: completed for %s, records count: %d, elapsed: %.3fs", code, len(records), time.time() - t0)
