      ...
      <div class="card-body">
        <div class="metrics-grid">
          <div class="metric-item-spec">
            ...
            <span class="val">{{ formatPercent(item.percentile) }}</span>
            <div v-if="item.percentile !== null" class="percentile-indicator">
              ...
              <div class="indicator-labels">
                <span class="buy-label">低位 (买)</span>
                <span>中位</span>
                <span class="sell-label">高位 (卖)</span>
              </div>
            </div>
          </div>
          ...
          <div class="metric-box">
            ...
            <span class="val text-warning">{{ formatPercent(item.max_drawdown) }}</span>
          </div>
          <div class="metric-box">
...
.advice-card { margin-bottom: 16px; padding: 4px 8px; }
.card-top {
  display: flex;
...
.name { font-weight: 700; font-size: 17px; color: var(--el-text-color-primary); transition: color 0.2s ease; }
...
.muted { color: var(--el-text-color-secondary) !important; font-weight: normal; }
...
.indicator-bar-bg {
  height: 5px;
  border-radius: 3px;
  background: linear-gradient(to right, var(--el-color-success) 0%, rgba(255,255,255,0.15) 50%, var(--el-color-danger) 100%);
  width: 100%;
}
.indicator-pointer {
  position: absolute;
  ...
  box-shadow: 0 0 6px rgba(255, 255, 255, 0.8), 0 0 4px var(--el-color-primary);
}
.indicator-labels {
  display: flex;
  justify-content: space-between;
...
.buy-label { color: var(--el-color-success-light-3); }
.sell-label { color: var(--el-color-danger-light-3); }
</style>
